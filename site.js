...
const linkIcon = label => ({
  ...
  LinkedIn: "<svg class=\"linkedin-logo\" viewBox=\"0 0 24 24\" aria-label=\"LinkedIn\"><rect x=\"3\" y=\"3\" width=\"18\" height=\"18\" rx=\"2\" class=\"icon-fill\"></rect><path class=\"linkedin-cutout\" d=\"M8.3 9.25H5.7V18h2.6V9.25ZM7 5.25a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM10.1 9.25V18h2.6v-4.33c0-1.14.22-2.25 1.63-2.25 1.39 0 1.4 1.3 1.4 2.32V18h2.6v-4.78c0-2.34-.5-4.14-3.23-4.14-1.31 0-2.18.72-2.54 1.41h-.04V9.25h-2.42Z\"></path></svg>",
...
function shell(page, content) {
  ...
  return `<div class="site-frame"><header class="site-header">${brand()}${navMarkup(page)}</header><main>${content}</main><footer class="site-footer"><div class="footer-intro">${brand()}<p>Made in Montréal, with curiosity.</p></div><div class="footer-group"><p class="footer-label">Explore</p><div class="footer-list">${sitePages.map(([, label, url]) => `<a href="${url}">${label}<span aria-hidden="true">→</span></a>`).join("")}</div></div><div class="footer-group"><p class="footer-label">Connect</p><div class="footer-list footer-contact">${links.map(link => `<a href="${link.url}" ${link.url.startsWith("mailto:") ? "" : "target=\"_blank\" rel=\"noreferrer\""}><span class="link-icon">${linkIcon(link.label)}</span>${link.label}</a>`).join("")}</div></div><div class="footer-meta"><small class="copyright">© ${new Date().getFullYear()} Alex Zhu. All rights reserved.</small><p>Student · maker · teammate</p></div></footer></div>`;
}
